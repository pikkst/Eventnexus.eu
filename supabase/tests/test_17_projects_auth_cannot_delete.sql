-- RLS test: Authenticated users cannot delete projects
...
BEGIN
  ...
  IF v_row_count > 0 THEN
    RAISE EXCEPTION 'Test 17 failed: authenticated users can delete projects (deleted % rows)', v_row_count;
  END IF;

  DELETE FROM public.projects WHERE id = v_project_id;
  DELETE FROM public.project_leads WHERE id = v_lead_id;
  DELETE FROM public.profiles WHERE id = v_user_id;
  RAISE NOTICE 'Test 17 passed: authenticated users cannot delete projects';
END;
$$;
