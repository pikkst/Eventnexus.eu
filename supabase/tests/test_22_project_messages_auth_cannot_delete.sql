-- RLS test: Authenticated users cannot delete project_messages
...
BEGIN
  ...
  IF v_row_count > 0 THEN
    RAISE EXCEPTION 'Test 22 failed: authenticated users can delete project_messages (deleted % rows)', v_row_count;
  END IF;
  ...
  RAISE NOTICE 'Test 22 passed: authenticated users cannot delete project_messages';
END;
$$;
