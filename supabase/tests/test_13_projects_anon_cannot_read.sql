-- RLS test: Anonymous users cannot read projects
...
BEGIN
  ...
  IF v_count != 0 THEN
    RAISE EXCEPTION 'Test 13 failed: anonymous users can read projects (count=%)', v_count;
  END IF;
  RESET ROLE;
  PERFORM set_config('request.jwt.claims', '{}', true);
  ...
  RAISE NOTICE 'Test 13 passed: anonymous users cannot read projects';
END;
$$;
