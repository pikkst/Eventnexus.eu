-- RLS test: Authenticated users cannot update projects
...
BEGIN
  ...
  UPDATE public.projects SET status = 'reviewed' WHERE id = v_project_id;
  GET DIAGNOSTICS v_row_count = ROW_COUNT;
  RESET ROLE;
  PERFORM set_config('request.jwt.claims', '{}', true);

  IF v_row_count > 0 THEN
    RAISE EXCEPTION 'Test 16 failed: authenticated users can update projects (updated % rows)', v_row_count;
  END IF;

  DELETE FROM public.projects WHERE id = v_project_id;
  DELETE FROM public.project_leads WHERE id = v_lead_id;
  DELETE FROM public.profiles WHERE id = v_user_id;
  RAISE NOTICE 'Test 16 passed: authenticated users cannot update projects';
END;
$$;
