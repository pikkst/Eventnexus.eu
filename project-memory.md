...
- Supabase usage decision: use Supabase first for `project_leads`, `projects`, `project_messages`, lead statuses, and later admin review data; full customer portal features remain future scope.
...
- Initial admin seed documentation lives at `supabase/seed-admin.md`; local development uses `npm run seed:local-admin` with `LOCAL_ADMIN_EMAIL` and `LOCAL_ADMIN_PASSWORD` from `.env.test`; `scripts/seed-local-admin.ts` includes a safety check and refuses to run if `SUPABASE_URL` is not a localhost/127.0.0.1 address; hosted provisioning uses the Dashboard SQL Editor flow described in `supabase/seed-admin.md`
...
- `projects` and `project_messages` schema and RLS policies are defined in `supabase/migrations/202608130001_create_projects.sql` and `supabase/migrations/202608130002_create_project_messages.sql`; both tables have deny-all RLS for `anon` and `authenticated` roles on SELECT/INSERT/UPDATE/DELETE; `service_role` has full access; admin reads/writes happen server-side through protected API routes using `SUPABASE_SERVICE_ROLE_KEY` after admin role verification; `projects` includes `lead_id` (unique reference to `project_leads`), `status` with lifecycle check constraint, `admin_notes`, `lead_score`, `next_action`, `follow_up_date`, `assigned_admin_id`, and `project_value_estimate`; `project_messages` includes `project_id` (cascading delete), `sender_type` (`admin` or `client`), `sender_email`, `message_type` with template check constraint, `subject`, `body`, `sent_via_email`, `email_sent_at`, and `email_error`; a follow-up migration `202608130003_add_project_messages_updated_at.sql` adds the missing `updated_at` column and fixes the BEFORE UPDATE trigger; RLS policies are documented in `supabase/admin-schema.sql`; database tests in `supabase/tests/test_13_projects_anon_cannot_read.sql` through `test_17_projects_auth_cannot_delete.sql` and `test_18_project_messages_anon_cannot_read.sql` through `test_22_project_messages_auth_cannot_delete.sql` cover all roles and operations
