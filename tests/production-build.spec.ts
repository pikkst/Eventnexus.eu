...
test.describe('production build behavior', () => {
  test('critical pages load from built output', async ({ page }) => {
    await page.goto('/en');
    await expect(page.locator('h1').first()).toContainText('Eventnexus builds your idea into a working web platform.');

    await page.goto('/en/services');
    await expect(page.locator('h1').first()).toContainText('Complete web-platform development from idea to launch.');
  ...
  test('canonical unprefixed routes redirect to English locale', async ({ page }) => {
    const routes = [
  ...
  test('contact-only submission succeeds on built output', async ({ page }) => {
    ...
    await page.fill('#contactMessage', 'This is a test message that meets the minimum length requirement for the built output test.');
  ...
  test('full project request form renders all steps on built output', async ({ page }) => {
    await page.goto('/en/contact');
    ...
    await page.fill('textarea[name="ideaDescription"]', 'This is a test description that is at least 80 characters long to pass validation.');
    await page.click('#next-btn');
