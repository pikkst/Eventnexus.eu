...
test.describe('analytics', () => {
  test('does not load beacon script when analytics is disabled', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('script[src*="static.cloudflareinsights.com/beacon.min.js"]')
    ).toHaveCount(0);
    await expect(page.locator('meta[name="ga4-id"]')).toHaveCount(0);
  });

  test('loads beacon script when analytics is enabled and consent is granted', async ({
    page,
  }) => {
  ...
  test('loads analytics on fresh page load with persisted granted consent', async ({
    page,
  }) => {
    if (test.info().project.name !== 'chromium-enabled') {
      test.skip(true, 'Analytics is disabled in this project');
  ...
  test('does not duplicate beacon script after consent change event', async ({
    page,
  }) => {
    if (test.info().project.name !== 'chromium-enabled') {
      test.skip(true, 'Analytics is disabled in this project');
  ...
  test('privacy settings button clears consent and reloads page', async ({
    page,
  }) => {
    if (test.info().project.name !== 'chromium-enabled') {
      test.skip(true, 'Analytics is disabled in this project');
